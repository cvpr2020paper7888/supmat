...
class CameraFieldOfViewTask(Task):
    """ Output space for point-info prediction tasks (what models do we evem use?) """

    def __init__(self, *args, **kwargs):

        self.point_type = kwargs.pop("point_type", "field_of_view_rads")
    ...
    def file_loader(self, path, resize=None, seed=None):
        points = json.load(open(path))[self.point_type]
        return points
...
tasks = [
    ...
    CameraFieldOfViewTask('point_info'),
]
