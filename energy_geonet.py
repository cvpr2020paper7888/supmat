...
energy_configs = {
    ...
    # Do not modify this config! Otherwise you'll have to modify GeoNetTaskGraph in graph.py
    'geonet': {
        "paths": {
            "rgb": [tasks.rgb],
            "normal": [tasks.normal],
            "depth": [tasks.depth_zbuffer],
            "N(rgb)": [tasks.rgb, tasks.normal],
            "D(rgb)": [tasks.rgb, tasks.depth_zbuffer],
            "N(D(rgb))": [tasks.rgb, tasks.depth_zbuffer, tasks.normal],
            "D(N(rgb))": [tasks.rgb, tasks.normal, tasks.depth_zbuffer],
    },
        "losses": {
            "mse": {
                ("train", "val"): [
...
#                             print(path1, path2)
                            if path1 == 'N(D(rgb))':
                                  path_loss = path_loss * 0.01
                            if path1 == 'D(N(rgb))':
                                  path_loss = path_loss * 0.5
                            loss[loss_type] += path_loss
    ...
    def plot_paths(self, graph, logger, realities=[], plot_names=None, prefix=""):
        ...
        for name, config in (plot_names or self.plots.items()):
            ...
            for reality in realities:
                ...
                # if images is None:
                    # images = [[] for _ in range(0, len(paths))]
#                 print('paths, reality', paths, reality)
                shape = list(path_values[list(path_values.keys())[0]].shape)
                shape[1] = 3
                for i, path in enumerate(paths):
                    X = path_values.get(path, torch.zeros(shape, device=DEVICE))
...
class GeoNetEnergyLoss(EnergyLoss):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.tasks = [tasks.rgb, tasks.FoV, tasks.normal, tasks.depth_zbuffer]

    def get_tasks(self, reality):
        from task_configs import tasks
        if reality in ['ood', 'test']:
            return [tasks.rgb]
        else:
            return [tasks.rgb, tasks.FoV, tasks.normal, tasks.depth_zbuffer]


    def compute_paths(self, graph, reality=None, paths=None):
        return graph.compute_all_paths(reality)
#         path_cache = {}
#         paths = paths or self.paths
#         print('----------')
#         print(paths)
